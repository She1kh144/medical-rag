...
def generate_answer(query: str, chunks: list):
    ...
    system_prompt = (
        ...
        "1. Если вопрос касается общего случая (например, 'доза для взрослых'), "
        ...
        "есть много специальных случаев (особые группы пациентов, конкретные заболевания, "
        "и т.п.) — общая информация почти всегда тоже там есть.\n"
        "2. Если в контексте есть прямой ответ на общий вопрос, используй его, даже если "
        "рядом упоминаются особые случаи. Особые случаи — это дополнения, а не замена "
        "общему ответу.\n"
        "3. Отвечай 'В предоставленных документах нет ответа на этот вопрос' ТОЛЬКО если "
